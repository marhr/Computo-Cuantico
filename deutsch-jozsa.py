...
constant = {'0' : [], '1' : [cirq.X(q2)]}

# Oracles for balanced functions
balanced = {'x1' : [cirq.CNOT(q0, q2)],
            'x2' : [cirq.CNOT(q1, q2)],
...
for key, oracle in constant.items():
    print('Circuito cuando Uf es {}...'.format(key))
...
for key, oracle in balanced.items():
    print('Circuito cuando Uf es {}...'.format(key))
    print(cirq.Circuit(your_circuit(oracle)), end="\n\n")
    result = simulator.run(cirq.Circuit(your_circuit(oracle)), repetitions=10)
    print(result)